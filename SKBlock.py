...
class SKBlock(nn.Module):
    ...
    def __init__(self, large_kernel, small_kernel, stride, out_channels):
        ...
        self.L = (large_kernel, large_kernel)
        self.S = (small_kernel, small_kernel)
        self.stride = stride
        self.out_channels
...
class DirectBlock(SKBlock):
    ...
    def forward(self, x):
        ...
        x1 = nn.Conv2d(in_channels=num_channels, out_channels=self.out_channels, kernel_size=self.L,
        ...
        x3 = nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=(1, 1),
                       stride=1, padding=padding)(x2)

        out_x = nn.Sigmoid()(x2 + x3)
...
class ParallelBlock(SKBlock):
    ...
    def forward(self, x):
        ...
        x1_L = nn.Conv2d(in_channels=num_channels, out_channels=self.out_channels, kernel_size=self.L,
                         stride=self.stride, groups=num_channels, padding=padding)(x)

        padding = pad_for_conv2d(H, W, self.S, stride=self.stride)

        x1_S = nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=self.S,
                         stride=self.stride, groups=num_channels, padding=padding)(x)
...
class ConicalBlock(SKBlock):
    ...
    def forward(self, x):
        ...
        x1_S = nn.Conv2d(in_channels=num_channels, out_channels=self.out_channels, kernel_size=self.S,
                         stride=self.stride, groups=num_channels, padding=padding)(x)
        ...
        x1_L = nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=self.L,
